...
class IrAttachment(models.Model):
    ...
    def _register_hook(self):
        ...
        # 'update_module' is an argument than 'load_modules' receive
        # with a list of addons to install / upgrade, if it's not
        # empty, we know that a -i/-u has been run and that we
        # need to move the attachments that could have been created
        # or updated in other addons before this addon was loaded
        ...
        # when we are upgrading addons, some of them might add attachments, and
        # to be sure the are migrated to the storage we need to call the
        # migration here.
        # Typical example is images of ir.ui.menu which are updated in
        # ir.attachment at every upgrade of the addons
        if update_module:
            env = api.Environment(self.env.cr, odoo.SUPERUSER_ID, {})
            env['ir.attachment']._force_storage_to_object_storage()
    ...
    def _move_attachment_to_store(self):
        ...
        if fname:
            ...
            # commit before deleting the file, so we won't lose
